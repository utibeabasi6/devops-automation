...
def gen_fluentbit(logging):
    env = Environment(
        loader=FileSystemLoader("templates"),
        autoescape=select_autoescape()
    )
    template = env.get_template("fluentbit.yaml.j2")
    print("Generating fluentbit config file".center(80, "-"))
...
# Generate the docker-compose file for fluentbit and pass in the ip address of the logging server
with open("ansible/files/fluentbit/docker-compose.yaml", "w") as f:
    f.write(gen_fluentbit(logging_ip))
...
# Generate a service file for fluentbit
with open("ansible/files/fluentbit/fluentbit.service", "w") as f:
    f.write(gen_systemd("fluentbit", "/app/start.sh", description="A systemd service for fluentbit"))
